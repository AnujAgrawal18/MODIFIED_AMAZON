...
export async function getWebPageBySlug(slug: string) {
  ...
  if (!webPage) {
    // Try to find in in-memory data as fallback
    const fallback = data.webPages?.find(
      (page) => page.slug === slug && page.isPublished
    )
    if (!fallback) throw new Error('WebPage not found')
